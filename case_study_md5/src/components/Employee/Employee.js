...
function Employee() {
  ...
  const handleSearch = () => {
    const newFilteredList = employeeList.filter(
      (item) =>
        item.firstName &&
  ...
  const getAll = async () => {
    try {
      const res = await method.getAllEmployeeList(
        currentPage + 1,
        itemsPerPage
      ); // phân trang
      if (res && res.data) {
    ...
    <>
      ...
      <div className="my-container">
        ...
        <button
          ...
          onClick={sortEmployeeByWage}
        >
          <LowPriorityIcon /> Sắp xếp theo lương
        </button>
        <input
          className="search"
          type="text"
